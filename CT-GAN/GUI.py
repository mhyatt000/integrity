...
class GUI(object):
    ...
    def _load_paths(self, path):
        ...
        for file in files:
            if os.path.isdir(file):
                ...
                elif (subdir_files[0].split(".")[-1] == "mhd") or (
                    subdir_files[0].split(".")[-1] == "raw"
                ):  # MHD
    ...
    def save(self, event):
        if self.savepath is not None:
            ...
            # save scan
            self.manipulator.save_tampered_scan(
                os.path.join(self.savepath, uuid), output_type="dicom"
            )
            # save coords
            file_exists = False
    ...
    def plot(self):
        ...
        if self.hist_state:
            self.im = self.ax.imshow(
                self.eq.equalize(self.manipulator.scan[self.ind, :, :]), cmap="bone"
            )  # , cmap="bone", vmin=-1000, vmax=1750)
        else:
            self.im = self.ax.imshow(
                self.manipulator.scan[self.ind, :, :], cmap="bone", vmin=-1000, vmax=1750
            )
